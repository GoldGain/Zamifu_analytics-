...
-- Official Drive file: 1jbAvVAWmif-toAfPShQm9UujN7bZ0luX (Grade 4 Science and Technology.pdf).
-- Exact hierarchy source: KICD viewer page 13 summary table and detailed page 14 heading.
...
SELECT s.id, v.strand_name, 'Official KICD Grade 4 Science and Technology revised-design strand.', v.strand_order
FROM (VALUES
...
SELECT st.id, v.sub_strand_name, 'Official KICD Grade 4 Science and Technology revised-design sub-strand.', v.sub_strand_order
FROM (VALUES
  ('Living Things and their Environment','Plants',1),
  ('Living Things and their Environment','Animals',2),
  ('Living Things and their Environment','Human digestive system',3),
  ('Matter','Properties of matter',1),
  ('Matter','Management of solid waste',2),
  ('Matter','Water conservation',3),
  ('Force and Energy','Force and its effects',1),
  ('Force and Energy','Light',2),
  ('Force and Energy','Heat',3)
...
SELECT ss.id, v.sub_strand_name, 'Official KICD Grade 4 Science and Technology revised-design content anchor from the exact summary-table sub-strand heading.', ARRAY[]::text[], 1
FROM (VALUES
  ('Living Things and their Environment','Plants'),
  ('Living Things and their Environment','Animals'),
  ('Living Things and their Environment','Human digestive system'),
  ('Matter','Properties of matter'),
  ('Matter','Management of solid waste'),
  ('Matter','Water conservation'),
  ('Force and Energy','Force and its effects'),
  ('Force and Energy','Light'),
  ('Force and Energy','Heat')
...
WHERE NOT EXISTS (
  ...
  WHERE t.sub_strand_id=ss.id
    ...
    AND t.topic_description ~* 'Official KICD Grade 4 Science and Technology'
);
