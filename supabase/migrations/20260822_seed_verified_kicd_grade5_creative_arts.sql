-- Source-verified KICD Grade 5 Creative Arts addition.
-- Official index: https://kicd.ac.ke/cbc-materials/curriculum-designs/grade-five-designs/
-- Official Drive file: 1euaxMgHyMqSm4qVSzOeJ_-9LXYgj6jyS (Creative Arts, revised 2024).
-- Production catalog alias is Creative Arts and Sports; source curriculum title is Creative Arts.
-- Additive/idempotent: legacy rows are preserved; no natural-key ON CONFLICT is used.
BEGIN;

INSERT INTO curriculum_subjects (grade_id, subject_name, subject_code)
SELECT g.id, 'Creative Arts and Sports', 'CAS'
FROM curriculum_grades g
WHERE g.grade_number=5
  AND NOT EXISTS (
    SELECT 1 FROM curriculum_subjects s
    WHERE s.grade_id=g.id
      AND lower(trim(s.subject_name)) IN ('creative arts','creative arts and sports')
  );

INSERT INTO curriculum_strands (subject_id, strand_name, strand_description, strand_order)
SELECT s.id, v.strand_name, 'Official KICD Grade 5 Creative Arts revised-design strand.', v.strand_order
FROM (VALUES
  ('Creating and Executing',1),('Performing and Displaying',2),('Appreciation in Creative Arts',3)
) v(strand_name,strand_order)
JOIN curriculum_grades g ON g.grade_number=5
JOIN curriculum_subjects s ON s.grade_id=g.id AND lower(trim(s.subject_name)) IN ('creative arts','creative arts and sports')
WHERE NOT EXISTS (
  SELECT 1 FROM curriculum_strands st
  WHERE st.subject_id=s.id
    AND lower(trim(st.strand_name))=lower(trim(v.strand_name))
    AND st.strand_description ~* '(official|source-verified)[[:space:]]+kicd'
);

INSERT INTO curriculum_sub_strands (strand_id, sub_strand_name, sub_strand_description, sub_strand_order)
SELECT st.id, v.sub_strand_name, 'Official KICD Grade 5 Creative Arts revised-design sub-strand.', v.sub_strand_order
FROM (VALUES
  ('Creating and Executing','Wind Musical instruments (Drawing)',1),('Creating and Executing','Football',2),('Creating and Executing','Rhythm',3),('Creating and Executing','Painting and Mosaic',4),('Creating and Executing','Melody',5),('Creating and Executing','Rounders',6),
  ('Performing and Displaying','Athletics',1),('Performing and Displaying','Fabric decoration',2),('Performing and Displaying','Kenyan Folk Dance',3),('Performing and Displaying','Puppetry',4),('Performing and Displaying','Descant Recorder',5),('Performing and Displaying','Swimming',6),('Performing and Displaying','Indigenous Kenyan Games',7),
  ('Appreciation in Creative Arts','Analysis of Creative Arts Works',1)
) v(strand_name,sub_strand_name,sub_strand_order)
JOIN curriculum_grades g ON g.grade_number=5
JOIN curriculum_subjects s ON s.grade_id=g.id AND lower(trim(s.subject_name)) IN ('creative arts','creative arts and sports')
JOIN curriculum_strands st ON st.subject_id=s.id AND lower(trim(st.strand_name))=lower(trim(v.strand_name)) AND st.strand_description ~* '(official|source-verified)[[:space:]]+kicd'
WHERE NOT EXISTS (
  SELECT 1 FROM curriculum_sub_strands ss
  WHERE ss.strand_id=st.id
    AND lower(trim(ss.sub_strand_name))=lower(trim(v.sub_strand_name))
    AND ss.sub_strand_description ~* '(official|source-verified)[[:space:]]+kicd'
);

INSERT INTO curriculum_topics (sub_strand_id, topic_name, topic_description, learning_objectives, topic_order)
SELECT ss.id, v.sub_strand_name, 'Official KICD Grade 5 Creative Arts revised-design content anchor.', ARRAY[]::text[], 1
FROM (VALUES
  ('Creating and Executing','Wind Musical instruments (Drawing)'),('Creating and Executing','Football'),('Creating and Executing','Rhythm'),('Creating and Executing','Painting and Mosaic'),('Creating and Executing','Melody'),('Creating and Executing','Rounders'),
  ('Performing and Displaying','Athletics'),('Performing and Displaying','Fabric decoration'),('Performing and Displaying','Kenyan Folk Dance'),('Performing and Displaying','Puppetry'),('Performing and Displaying','Descant Recorder'),('Performing and Displaying','Swimming'),('Performing and Displaying','Indigenous Kenyan Games'),
  ('Appreciation in Creative Arts','Analysis of Creative Arts Works')
) v(strand_name,sub_strand_name)
JOIN curriculum_grades g ON g.grade_number=5
JOIN curriculum_subjects s ON s.grade_id=g.id AND lower(trim(s.subject_name)) IN ('creative arts','creative arts and sports')
JOIN curriculum_strands st ON st.subject_id=s.id AND lower(trim(st.strand_name))=lower(trim(v.strand_name)) AND st.strand_description ~* '(official|source-verified)[[:space:]]+kicd'
JOIN curriculum_sub_strands ss ON ss.strand_id=st.id AND lower(trim(ss.sub_strand_name))=lower(trim(v.sub_strand_name)) AND ss.sub_strand_description ~* '(official|source-verified)[[:space:]]+kicd'
WHERE NOT EXISTS (
  SELECT 1 FROM curriculum_topics t
  WHERE t.sub_strand_id=ss.id
    AND lower(trim(t.topic_name))=lower(trim(v.sub_strand_name))
    AND t.topic_description ~* 'Official KICD Grade 5 Creative Arts'
);

COMMIT;
