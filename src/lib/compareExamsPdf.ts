...
import { addSignaturesToPDF, drawReportFooter, type SchoolInfo, type SignatureInfo } from '@/lib/reportCardPdf';
...
function makeSide(rows: any[], label: string, termLabel: string, examLabel: string, band: any): ComparisonSide {
  ...
  const learners = Array.from(byStudent.values()).map((learner) => {
    ...
    return { ...learner, total, average, grade: band === 'primary' ? gradeResult.grade : gradeResult.subLevel };
...
export async function generateComparisonPdf(data: ComparisonData, schoolInfo: SchoolInfo, signatures: SignatureInfo, fontSize: PdfFontSize = 14) {
  ...
  const top = Array.from({ length: Math.max(10, data.sideA.learners.length, data.sideB.learners.length) }, (_, i) => [String(i + 1), data.sideA.learners[i]?.name || '—', data.sideA.learners[i]?.stream || '—', data.sideA.learners[i]?.total.toFixed(0) || '—', data.sideB.learners[i]?.name || '—', data.sideB.learners[i]?.stream || '—', data.sideB.learners[i]?.total.toFixed(0) || '—']).slice(0, 10); addTable(doc, ['POS', `${data.sideA.examLabel} Learner`, 'Stream', 'Total', `${data.sideB.examLabel} Learner`, 'Stream', 'Total'], top, 28, fontSize);

  nextPage(doc, 'COMPARE EXAMS — SECTION 4: SUBJECT PERFORMANCE', subtitle);
  const subjectRows = data.sideA.subjects.map((subject) => { const av = data.sideA.learners.map((l) => l.subjects[subject]).filter((v) => v != null); const bv = data.sideB.learners.map((l) => l.subjects[subject]).filter((v) => v != null); const a = mean(av), b = mean(bv); return [subject, a == null ? '—' : a.toFixed(1), b == null ? '—' : b.toFixed(1), signed(a != null && b != null ? b - a : null)]; }); addTable(doc, ['Learning Area', data.sideA.examLabel + ' Mean', data.sideB.examLabel + ' Mean', 'Change'], subjectRows, 28, fontSize, true);
  ...
  addTable(doc, ['POS', data.sideA.examLabel, 'Stream', 'Total'], data.sideA.learners.slice(0, 10).map((l, i) => [i + 1, l.name, l.stream, l.total.toFixed(0)]), 28, fontSize); addTable(doc, ['POS', data.sideB.examLabel, 'Stream', 'Total'], data.sideB.learners.slice(0, 10).map((l, i) => [i + 1, l.name, l.stream, l.total.toFixed(0)]), 145, fontSize);
  ...
  const last = doc.getNumberOfPages(); doc.setPage(last); await addSignaturesToPDF(doc, signatures, 245, schoolInfo); drawReportFooter(doc);
