...
function normalizeQuestion(raw: unknown, fallbackType: QuestionType, fallbackDifficulty: 'easy' | 'medium' | 'hard'): GeneratedExamQuestion | null {
  ...
  return {
    ...
    visual_spec: item.visual_spec && typeof item.visual_spec === 'object'
      ...
      : (toSafeString(item.image_prompt) ? { asset_type: 'illustration', prompt: toSafeString(item.image_prompt).slice(0, 500) } : null),
  };
}
