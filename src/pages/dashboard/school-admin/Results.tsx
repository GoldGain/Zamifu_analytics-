...
function computeSubjectGradeStats(summaries: any[], subject: string, band: SchoolLevelBand) {
  const labels = gradeLabelsForBand(band);
  ...
  summaries.forEach((summary: any) => {
    ...
    const label = band === 'primary' ? grade.grade : grade.subLevel;
...
export default function SchoolAdminResults({ scope = 'school' }: { scope?: ResultsScope }) {
  ...
  const buildStudentSummary = (rawResults: any[], classObj: any) => {
    ...
    const configuredAreaCount = new Set(rawResults.map((r: any) => normalizeLearningAreaName(r.subjects?.name || '')).filter((name) => name && name !== 'Unknown')).size;
  ...
  const downloadClassResultsPDF = async (fontSize: PdfFontSize = DEFAULT_PDF_FONT_SIZE) => {
    ...
    try {
      ...
      doc.addPage('a4', 'portrait');
      {
        ...
        const topRows = computeTopLearnersPerArea(summaries, allSubjects, band, 10);
      ...
      doc.addPage();
      {
        ...
        const analysisGradeLabels = gradeLabelsForBand(band);
        const subRows = subjectStats.map((s, i) => {
          const stats = computeSubjectGradeStats(summaries, s.name, band);
  ...
  const renderCompactStreamSummary = async (doc: jsPDF, opts: { classObj: any; label: string; rawResults: any[]; termObj: any; assessmentLabel: string; previousTerm: any; previousSubjectStats: Map<string, number>; previousDistribution: Map<string, number>; previousTotalStudents: number | null; fontSize: PdfFontSize }) => {
    ...
    const requiredLearningAreas = getRequiredLearningAreas(opts.classObj, allSubjects.length) ?? allSubjects.length;
    ...
    const gradeLabels = gradeLabelsForBand(band);
    const gradeDistribution = allSubjects.map((subject) => {
      const stats = computeSubjectGradeStats(summaries, subject, band);
    ...
    // This is intentionally the same table shape used by the All-Streams PDF;
    // the only omitted column is Stream because this report is already scoped
    // to one stream (or a no-stream class).
    doc.addPage('a4', 'portrait');
    doc.setFillColor(245, 166, 35); doc.rect(0, 0, 210, 20, 'F');
    doc.setTextColor(26, 35, 126); doc.setFontSize(pdfFontSize(doc, 13)); doc.setFont('helvetica', 'bold');
    doc.text('LEARNER RESULTS', 105, 12, { align: 'center' });
    const learnerHeaders = isPrimary
      ? ['POS', 'Learner', ...allSubjects.map((s) => shortName(s)), 'Total', 'Avg%', 'Grade']
      : ['POS', 'Learner', ...allSubjects.map((s) => shortName(s)), 'Total', 'Avg%', 'Pts', 'Grade'];
    const learnerBody = summaries.map((s: any) => {
      const grade = overallGradeWithBand(s.avgPct, band);
      const cells = allSubjects.map((subject) => {
        const pct = s.subjects[subject];
        return pct === undefined ? '—' : `${Number(pct).toFixed(0)}% ${overallGradeWithBand(Number(pct), band).subLevel}`;
      });
      const row: any[] = [String(s.position), `${s.student?.first_name || ''} ${s.student?.last_name || ''}`, ...cells, String(Math.round(s.totalPct || 0)), `${Number(s.avgPct || 0).toFixed(1)}%`];
      if (!isPrimary) row.push(String(s.totalPoints || 0));
    ...
    // ── STREAM COMPARISON BY TOTAL MEAN MARKS ────────────────────────────────
    {
      ...
      const streamRows = Array.from(acc.values()).map((e) => {
        ...
        return { label, mean: e.n > 0 ? e.totalPct / e.n : 0, outOf: requiredLearningAreas * 100 };
    ...
    const headers = isPrimary ? ['POS', 'Learner', 'Stream', ...subjectShorts, 'Total', 'Avg%', 'Grade'] : ['POS', 'Learner', 'Stream', ...subjectShorts, 'Total', 'Avg%', 'Pts', 'Grade'];
