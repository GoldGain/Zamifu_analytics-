...
export default function SchoolAdminResults({ scope = 'school' }: { scope?: ResultsScope }) {
  ...
  const renderCompactStreamSummary = async (doc: jsPDF, opts: { classObj: any; label: string; rawResults: any[]; termObj: any; assessmentLabel: string; previousTerm: any; previousSubjectStats: Map<string, number>; previousDistribution: Map<string, number>; previousTotalStudents: number | null; fontSize: PdfFontSize }) => {
    ...
    const gradeDistribution = allSubjects.map((subject) => {
      const counts: string[] = gradeLabels.map((label) => {
        const n = summaries.filter((s: any) => {
          const pct = s.subjects[subject];
          if (pct === undefined) return false;
          const gr = overallGradeWithBand(Number(pct), band);
          return (isPrimary ? gr.grade : gr.subLevel) === label;
        }).length;
        return String(n);
      });
      return [subject.length > 18 ? `${subject.slice(0, 17)}…` : subject, ...counts];
    });
    autoTable(doc, {
      startY: 30,
      head: [['Learning Area', ...gradeLabels]],
