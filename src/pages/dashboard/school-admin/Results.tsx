...
export default function SchoolAdminResults({ scope = 'school' }: { scope?: ResultsScope }) {
  ...
  const downloadClassResultsPDF = async (fontSize: PdfFontSize = DEFAULT_PDF_FONT_SIZE) => {
    ...
    try {
      ...
      // ── PAGE 1: CLASS SUMMARY ────────────────────────────────────────────────────
      {
        ...
        const numSubjects = getRequiredLearningAreas(classObj) ?? allSubjects.length;
  ...
  const renderCompactStreamSummary = async (doc: jsPDF, opts: { classObj: any; label: string; rawResults: any[]; termObj: any; assessmentLabel: string; previousTerm: any; previousSubjectStats: Map<string, number>; previousDistribution: Map<string, number>; previousTotalStudents: number | null; fontSize: PdfFontSize }) => {
    ...
    doc.setTextColor(255, 255, 255); doc.setFont('helvetica', 'bold'); doc.setFontSize(pdfFontSize(doc, 14));
    doc.text(schoolInfo.name || schoolName || 'School', 105, 8, { align: 'center' });
    doc.setFontSize(pdfFontSize(doc, 10)); doc.text('SUBJECT ANALYSIS BY GRADES', 105, 16, { align: 'center' });
    const analysisLabels = gradeLabelsForBand(band);
    const analysisRows = allSubjects.map((subject) => {
      const stats = computeSubjectGradeStats(summaries, subject, band);
      return [subject, ...analysisLabels.map((label) => String(stats.counts[label] || 0)), String(stats.totalLearners), stats.average.toFixed(2)];
    }).sort((a, b) => Number(b[b.length - 1]) - Number(a[a.length - 1]));
    autoTable(doc, { startY: 28, head: [['Learning Area', ...analysisLabels, 'Total', 'Average']], body: analysisRows, styles: { fontSize: pdfFontSize(doc, 7), cellPadding: 1.5, halign: 'center' }, headStyles: { fillColor: [106, 27, 154], textColor: 255, fontSize: pdfFontSize(doc, 7), fontStyle: 'bold' }, alternateRowStyles: { fillColor: [232, 234, 246] }, columnStyles: { 0: { halign: 'left', cellWidth: 40 } }, margin: { left: 10, right: 10 } });
    doc.setFontSize(pdfFontSize(doc, 7)); doc.setTextColor(150, 150, 150); doc.text('Average uses weighted grade points divided by learners who took the learning area.', 105, 284, { align: 'center' });
    doc.text('Generated by Zamifu Analytics School Management System', 105, 290, { align: 'center' });
    ...
    // ── FINAL PAGE: LEARNER PERFORMANCE RESULTS FOR ALL LEARNERS ─────────────
    // Keep this table last so the report ends with the complete learner results.
    doc.addPage('a4', 'portrait');
    {
      ...
      const tableRows = summaries.map((s: any) => {
        ...
        const subjectCells = allSubjects.map(sub => {
          ...
          return `${subPct.toFixed(0)}% ${overallGradeWithBand(subPct, band).subLevel}`;
        });
